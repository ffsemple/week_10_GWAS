...
no_case_controls <- pheno %>% count(MDD) %>% 
  kable()
no_case_controls
...
```{r}
pheno_chr11 <- full_join(genotypes_chr11,pheno)
pheno_chr20 <- full_join(genotypes_chr20,pheno)
```
...
no significant effect of genotype on height. Effect size is very small (-0.1475), likely a result due to chance
...
```{r}
 # Order data by chromosome and base pair position
 height_sumstats_chr20 <- height_sumstats_chr20[order(height_sumstats_chr20$CHR, height_sumstats_chr20$POS),]

# Manhattan plot with ggplot2
ggplot(height_sumstats_chr20, aes(x = POS, y = -log10(P))) +
  geom_point(size = 1.2, alpha = 0.8) +
  scale_color_manual(values = c("steelblue", "skyblue")) +  # Alternating colors
  labs(x = "Base Pair", y = "-log10(p-value)") +
  theme_minimal() +
  theme(
    legend.position = "none",
    axis.text.x = element_text(angle = 45, hjust = 1)
  ) +
  geom_hline(yintercept = -log10(5e-8), linetype = "dashed", color = "red")  # Genome-wide significance line
...
ch20 and this is downstream of GDF5 (antisense RNA) - responsible for cartilage development and the SNP is associated with shorter height in women
